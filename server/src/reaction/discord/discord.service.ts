...
export class DiscordService {
  constructor(
    ...
    this.eventEmitter.on(
      'sendMessageDiscord.struct',
      (struct: DiscordSendMessageDto) => {
    ...
    this.eventEmitter.on(
      'send message user.struct',
      (struct: DiscordSendMessageUserDto) => {
      ...
      .then((res) => {
        res.data.forEach(async (element) => {
          if (element.name === struct.name_server) {
            await axios
              .get(
                `https://discord.com/api/v10/guilds/${element.id}/channels`,
                {
                  headers: {
                    Authorization: `Bot ${process.env.DISCORD_BOT_TOKEN}`,
                  },
              ...
              .then((res) => {
                res.data.forEach(async (element) => {
                  if (element.name === struct.name_channel) {
                    await axios
                      .post(
                        `https://discord.com/api/v10/channels/${element.id}/messages`,
                        {
  ...
  async sendMessageUser(struct: DiscordSendMessageUserDto, token: string) {
    ...
    await axios
      .post(
        `https://discord.com/api/v10/channels/${res.data.id}/messages`,
        {
          content: struct.content,
        },
        {
